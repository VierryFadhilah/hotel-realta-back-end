...
export class RestoMenuPhotosService {
    ...
    try {
      ...
      for (let i = 0; i < foto.length; i++) {
        const photoMenu = await this.photoModel.create({
          ...
          remp_primary: createRestoMenuPhotoDto.remp_primary,
          remp_url: `http://localhost:${process.env.PORT}/resto_menu_photos/uploads/image/resto/${foto[i].filename}`,
          remp_reme_id: createRestoMenuPhotoDto.remp_reme_id,
        });
        data.push(photoMenu);
      }
      return { message: `Data berhasil ditambahkan`, data };
    } catch (error) {
      return error;
    }
  }
  ...
  async findAll(): Promise<any> {
    try {
      ...
      return { message: `Data telah di temukan`, data: result };
    } catch (error) {
      return error;
    }
  }
  ...
  async update(
    ...
    try {
      ...
      return { message: `Data dengan remp_id ${remp_id} berhasil diupdate` };
    } catch (error) {
      console.error(error);
      return { message: `Data dengan remp_id ${remp_id} tidak di temukan` };
    }
  }
  ...
  async remove(remp_id: number): Promise<any> {
    try {
      ...
      return `Data dengan id ${remp_id} terhapus`;
    } catch (error) {
